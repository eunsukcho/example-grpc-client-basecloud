...
func RunHostAgent() {
	fmt.Println("🖥️ Host Agent 클라이언트 시작...")
...
func receiveMessages(stream proto.Receiver_ConnectClient) {
	for {
		...
		case "command":
			fmt.Printf("⚡ 명령 수신: %s\n", msg.GetContent())
			// 명령 실행 후 응답
			response := createMessage("host-agent-client", "command_response", 
				fmt.Sprintf("Host 명령 실행 완료: %s", msg.GetContent()))
			stream.Send(response)
		case "status_request":
			fmt.Println("📊 상태 요청 수신")
			statusResponse := createMessage("host-agent-client", "status_response", 
				"Host CPU: 30%, Memory: 70%, Disk: 55%")
			stream.Send(statusResponse)
		case "config_update":
			fmt.Printf("⚙️ 설정 업데이트 수신: %s\n", msg.GetContent())
			configResponse := createMessage("host-agent-client", "config_ack", 
				"Host 설정 업데이트 완료")
			stream.Send(configResponse)
		case "heartbeat":
			fmt.Println("💓 하트비트 수신")
			heartbeatResponse := createMessage("host-agent-client", "heartbeat_ack", "host_alive")
...
func sendVariousMessages(stream proto.Receiver_ConnectClient) {
	agentID := "host-agent-client"

	// 1. 연결 메시지 전송
	connectMsg := createMessage(agentID, "connect", "Host Agent 클라이언트 연결")
	...
	// 2. 초기화 메시지 전송
	initMsg := createMessage(agentID, "init", "Host 클라이언트 초기화 완료")
	...
	// 3. 등록 메시지 전송
	registerMsg := createMessage(agentID, "register", "agent_type=host,version=1.0.0,os=windows")
	...
	// 4. 설정 요청 메시지 전송
	configRequestMsg := createMessage(agentID, "config_request", "Host 설정 정보 요청")
	...
	// 5. 상태 보고 메시지 전송
	statusMsg := createMessage(agentID, "status_report", "Host 정상 동작 중")
	...
	// 주기적으로 다양한 메시지 전송
	ticker := time.NewTicker(15 * time.Second) // Host는 15초마다
	defer ticker.Stop()
	...
	for {
		...
		case <-ticker.C:
			...
			case 0:
				// ping 메시지
				pingMsg := createMessage(agentID, "ping", "host_ping")
			...
			case 1:
				// 하트비트 메시지
				heartbeatMsg := createMessage(agentID, "heartbeat", "host_heartbeat")
			...
			case 2:
				// 로그 메시지
				logMsg := createMessage(agentID, "log", fmt.Sprintf("Host 로그 메시지 #%d", messageCounter))
			...
			case 3:
				...
				metricMsg := createMessage(agentID, "metric", 
					fmt.Sprintf("host_cpu_usage=%d,host_memory_usage=%d,host_disk_usage=%d", 
			...
			case 4:
				// 이벤트 메시지
				eventMsg := createMessage(agentID, "event", fmt.Sprintf("Host 이벤트 발생 #%d", messageCounter))
			...
			case 5:
				// 알림 메시지
				alertMsg := createMessage(agentID, "alert", fmt.Sprintf("Host 알림 #%d", messageCounter))
